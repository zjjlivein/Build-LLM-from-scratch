...
| 日志现象 | 问题标签 | 修复建议 |
|----------|----------|----------|
| Error: Process completed with exit code 1 | Develop PR 未合入 | 在描述里写上 PR 号，格式如下：`Merged： #3639` 出现蓝色链接 回车，会自动触发流水线，不要直接写链接。 |
...
| 日志现象 | 问题标签 | 修复建议 |
|----------|----------|----------|
| 没有过 | 没有设置Required | 没有设置Required,暂时不关注 |
